...
campaign = sem.CampaignManager.new(ns_path, ns_script, ns_res_path, optimized=True,
                                   runner_type='ParallelRunner', check_repo=False, overwrite=False)
...
# first_quart_del = []
# # Non centralized scheduler results
# for allocPeriod in params_orig['numIabs']:
#     print(f'Allocation period: {allocPeriod} - non cen')
#     params_grid.update (centralizedSched=[False], packetSize=100, weightPolicy=1, eta=[50], k=[2], muThreshold=[10], numIabs=allocPeriod)
#     results = campaign.db.get_results(params_grid)
#     thr_ue = get_e2e_ue_throughput(results, UDP_APP_TRACEFILE)
#     del_ue = get_e2e_ue_latency(results, UDP_APP_TRACEFILE)
#     first_quart_thr.append(np.percentile(thr_ue, 25))
#     first_quart_del.append(np.percentile(del_ue, 75))
# ax_first_quart_thr.plot(params_orig['numIabs'], first_quart_thr, label='Non cen')
# ax_first_quart_del.plot(params_orig['numIabs'], first_quart_del, label='Non cen')
# # Centralized scheduler results
# for policy in params_orig['weightPolicy']:
#     first_quart_thr = []
#     first_quart_del = []
#     for allocPeriod in params_orig['numIabs']:
#         print(f'Allocation period: {allocPeriod} - Policy: {policy}')
#         params_grid.update (centralizedSched=[True], packetSize=100, weightPolicy=policy, eta=[50], k=[2], muThreshold=[10], numIabs=allocPeriod)
#         results = campaign.db.get_results(params_grid)
#         thr_ue = get_e2e_ue_throughput(results, UDP_APP_TRACEFILE)
#         del_ue = get_e2e_ue_latency(results, UDP_APP_TRACEFILE)
#         first_quart_thr.append(np.percentile(thr_ue, 25))
...
#     ax_first_quart_thr.plot(params_orig['numIabs'], first_quart_thr, label=f'Policy {policy}')
#     ax_first_quart_del.plot(params_orig['numIabs'], first_quart_del, label=f'Policy {policy}')
