...
void 
EpcSgwPgwApplication::UeInfo::SetIab(bool iab)
{
  m_iab = true;
}

bool
EpcSgwPgwApplication::UeInfo::GetIab()
{
  return m_iab;
}
...
void 
EpcSgwPgwApplication::AddIab (uint16_t cellId, uint64_t imsi)
{
  NS_LOG_FUNCTION (this << cellId << imsi);
  EnbInfo enbInfo;
  enbInfo.imsi = imsi;
  enbInfo.iab = true;
  m_enbInfoByCellId[cellId] = enbInfo;

  m_iabCellIdByImsi[imsi] = cellId;
...
EpcSgwPgwApplication::DoCreateSessionRequest (EpcS11SapSgw::CreateSessionRequestMessage req)
{
  ...
  {
    // TODOIAB get the first wired eNB address and set it as the address 
    NS_FATAL_ERROR("Error: an IAB dev should not be considered as the P2P fw device");
  }
