...
{
  NS_LOG_LOGIC("Number of IEs " << m_numberOfIes);
  if(m_numberOfIes == 0)
  {
    NS_LOG_LOGIC("--------||||||||||||||||----------- Try to deserialize an EpcS1APHeader which is not an EpcS1APHeader! " << m_numberOfIes);
    return 0;
  }
...
EpcS1APHeader::Serialize (Buffer::Iterator start) const
{
  ...
  NS_LOG_LOGIC("Serialize m_procedureCode " << (uint32_t)m_procedureCode);
...
EpcS1APHeader::Deserialize (Buffer::Iterator start)
{
  ...
  NS_LOG_LOGIC("m_procedureCode " << (uint32_t)m_procedureCode);

  i.ReadU8 ();
  m_lengthOfIes = i.ReadU8 ();
  NS_LOG_LOGIC("m_lengthOfIes " << m_lengthOfIes);
  if((int)m_lengthOfIes - 3 < 0)
  {
    NS_LOG_LOGIC("Int overflow");
    // there was an overflow, and probably this is not an EpcS1APHeader
    // return 0
    m_numberOfIes = 0;
...
EpcS1APInitialUeMessageHeader::Deserialize (Buffer::Iterator start)
{
  ...
  for(uint16_t index = 0; index < m_numParentImsi; ++index)
  {
    m_parentImsiList.push_back(i.ReadNtohU64());
    m_headerLength += 8;
  }
  m_numberOfIes++;
...
std::list<uint64_t>
EpcS1APInitialUeMessageHeader::GetParentImsiList() const
{
  return m_parentImsiList;
}

void
EpcS1APInitialUeMessageHeader::SetParentImsiList(std::list<uint64_t> list)
{
  m_parentImsiList = list;
  m_numParentImsi = list.size();
  m_headerLength += list.size()*8;
}

void
EpcS1APInitialUeMessageHeader::AddParent(uint64_t imsi)
{
  m_parentImsiList.push_back(imsi);
  m_numParentImsi++;
  m_headerLength += 8;
}
...
EpcS1APInitialContextSetupRequestHeader::EpcS1APInitialContextSetupRequestHeader ()
  : m_numberOfIes (1 + 1 + 1 + 1),
    m_headerLength (9 + 3 + 9 + 4 + 1 + 1), // the last + 1 is for the bool IAB
    m_enbUeS1Id (0xfffa),
    m_mmeUeS1Id (0xfffffffa),
    m_iab(false)
{
...
bool 
EpcS1APInitialContextSetupRequestHeader::GetIab () const
{
  return m_iab;
}

void 
EpcS1APInitialContextSetupRequestHeader::SetIab (bool iab)
{
  m_iab = iab;
}
