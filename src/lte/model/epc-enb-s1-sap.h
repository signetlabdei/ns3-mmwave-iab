...
public:
  ...
  {
    DataRadioBearerSetupRequestParameters() : iab(false) {}
  ...
  struct NotifyNumIabPerRntiParameters
  {
    NotifyNumIabPerRntiParameters()  : numIab(0) {}
    uint16_t numIab;
    uint16_t rnti;
    bool iab;
  };
  ...
  /**
   * Propagate the information on the number of "children" IAB nodes associated to a certain RNTI
   */
  virtual void NotifyNumIabPerRnti (NotifyNumIabPerRntiParameters params) = 0;
...
public:
  ...
  virtual void NotifyNumIabPerRnti (NotifyNumIabPerRntiParameters params);
...
template <class C>
void MemberEpcEnbS1SapUser<C>::NotifyNumIabPerRnti (NotifyNumIabPerRntiParameters params)
{
  m_owner->DoNotifyNumIabPerRnti (params);
}
