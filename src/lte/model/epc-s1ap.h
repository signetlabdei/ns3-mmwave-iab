...
{
public:
  S1apIfaceInfo (Address remoteAddr, Ptr<Socket> localCtrlPlaneSocket);
  virtual ~S1apIfaceInfo (void);

  S1apIfaceInfo& operator= (const S1apIfaceInfo &);

public:
  Address   m_remoteIpAddr;
...
public:
  ...
  EpcS1apEnb (Ptr<Socket> localSocket, PacketSocketAddress epcIabApplicationAddress, uint16_t cellId, uint16_t mmeId);
  ...
  void AddS1apInterface (uint16_t enbId, uint16_t mmeId, Ptr<Socket> localS1apSocket, PacketSocketAddress epcIabApplicationAddress);
...
protected:
  ...
  ;
  virtual void DoForwardIabS1apMessage(Ptr<Packet> packet, uint64_t imsi);
...
private:
  ...
  std::map<uint64_t, bool> m_remoteImsiRequestMap;
