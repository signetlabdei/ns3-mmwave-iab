...
public:
  ...
  /** 
   * Add a new IAB node to the MME. 
   * \param ecgi E-UTRAN Cell Global ID, the unique identifier of the IAB as cellId
   * \param the imsi
   */
  void AddIab (uint16_t ecgi, uint64_t imsi);
...
private:
  ...
  void DoInitialUeMessage (EpcS1apSap::InitialUeMessageParams params);
  ...
  struct UeInfo : public SimpleRefCount<UeInfo>
  {
    ...
    uint16_t wiredId; // cellId of the first wired gNB (it could be cellId)
  };
  ...
  struct EnbInfo : public SimpleRefCount<EnbInfo>
  {
    uint16_t gci;
    Ipv4Address s1uAddr;
    // the following are for IAB nodes
    uint64_t imsi;
    bool iab;
  };
  ...
  // IAB cellId stored by imsi
  std::map<uint64_t, uint16_t> m_iabCellIdByImsi;
