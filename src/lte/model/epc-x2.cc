...
void
EpcX2::AddX2LocalInterface (uint16_t localCellId, Ptr<Socket> localSocket)
{
  NS_LOG_FUNCTION (this << localCellId << localSocket);
  
  // Get local eNB where this X2 entity belongs to
  // Ptr<Node> localEnb = GetObject<Node> ();

  // Create X2-C, X2-U socket for the local eNB
  
  // TODOIAB setup everything (recvcallback)
  // TODOIAB how to distinguish between X2-U and X2-C? We could add an UDP header with the correct port
}
