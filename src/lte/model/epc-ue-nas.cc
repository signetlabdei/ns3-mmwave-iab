...
EpcUeNas::Send (Ptr<Packet> packet)
{
  ...
  switch (m_state)
    ...
    case ACTIVE:
      {
        ...
        NS_LOG_INFO("EpcUeNas ACTIVE SendPacket imsi " << m_imsi << " bid " << (uint16_t)bid);
        if (bid == 0)
          {
            return false;
          }
    ...
    default:
      NS_LOG_WARN (this << " imsi " << m_imsi << " NAS OFF, discarding packet. Save into m_bufferReceivedNasOff");
      m_bufferReceivedNasOff.push_back(packet);
      return false;
      break;
    }
}
...
void
EpcUeNas::DoNotifyBearerSetup ()
{
  NS_LOG_FUNCTION(this);
  switch(m_state)
  {
    case ACTIVE:
      if(m_bufferReceivedNasOff.size() > 0)
      {
        NS_LOG_INFO("Bearer activated in LteUeRrc, send packets which were standing in the buffer");
        for (auto pck : m_bufferReceivedNasOff)
        {
          Send(pck);
        }
      }
    ...
    default:
      NS_LOG_WARN("Bearer activated but EpcUeNas not active " << m_state);
      break;
  }
}
